                         ...
                         const std::vector<int> &class_filter)
{
    ...
    // Отбрасываем детекции в серой letterbox-полосе (ложные рамки от (0,0))
    const LetterboxLayout lb = letterbox_layout(img_w, img_h);
    proposals.erase(
        std::remove_if(proposals.begin(), proposals.end(),
                       [&lb](const Object &o) {
                           return !letterbox_center_inside(o.x, o.y, o.w, o.h, lb);
                       }),
        proposals.end());
    ...
    float pad_w = lb.pad_w;
    float pad_h = lb.pad_h;
